...
describe('Vault', () => {
  ...
  it('checkOut - Native', async () => {
    ...
    const {hash, origin, token, name, symbol, decimals, amount, owner, from, to, tag} = iface.parseLog(logs[0]).args
  ...
  it('checkOut - Token', async () => {
    ...
    await erc20.mint(account, 1e+18)
    ...
    const {hash, origin, token, name, symbol, decimals, amount, owner, from, to, tag} = iface.parseLog(logs[1]).args
