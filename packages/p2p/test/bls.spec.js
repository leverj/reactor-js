...
describe('bls', function () {
  ...
  it('generate random keys and sign', async function () {
    ...
    let message = fromString('Hello Gluon 2')
    ...
    expect(toString(publicKey.toBytes())).toEqual(toString(pk))
    expect(bls.verify(pk, message, sig)).toEqual(true)

  })
  //use bls.aggregate api
  it.only('verifies aggregate signatures, technique 1', async function () {
    const sks = [bls.SecretKey.fromKeygen(), bls.SecretKey.fromKeygen()]//, bls.SecretKey.fromKeygen(), bls.SecretKey.fromKeygen()]
    const msgs = [[12, 13, 14]]
  ...
  it.skip('generate keys from json and sign', async function () {
    ...
    const node1 = await newNode({ port: 9002, peerIdJson })
