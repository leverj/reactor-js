...
describe.only('e2e', () => {
  ...
  it('should create new nodes, connect and init DKG', async () => {
    ...
    await POST(LEADER_PORT, 'dkg/start')
  ...
  it('aggregate signatures over pubsub topic', async () => {
    ...
    await POST(LEADER_PORT, 'tss/aggregateSign', {msg: txnHash})
    await setTimeout(100)
    expect(await GET(LEADER_PORT, `tss/aggregateSign?txnHash=${txnHash}`).then(_ => _.verified)).toEqual(true)
  })

  it('whitelist', async () => {
    const ports = await createApiNodes(4, false)
    await GET(LEADER_PORT + 1, 'peer/bootstrapped')
