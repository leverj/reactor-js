...
export const LEADER_PORT = 9000

const dirPath = (i) => `${e2ePath}/${i}`
const filePath = (i) => `${dirPath(i)}/info.json`
export const getInfo = (port) => JSON.parse(readFileSync(filePath(port - LEADER_PORT)).toString())
// export const getInfo = (port) => store.get(port)
...
export async function createApiNodesFrom(ports, howMany) {
  async function createApiNode(port) {
    const index = port - LEADER_PORT
    const bootstrapNodes = port === LEADER_PORT ?
      [] :
      await tryAgainIfError(_ => getInfo(LEADER_PORT).p2p.id).then(_ => [`/ip4/${externalIp}/tcp/10000/p2p/${_}`], tryCount)
...
export const publishWhitelist = async (ports, total, available) =>
  tryAgainIfError(_ => POST(LEADER_PORT, 'whitelist/publish')).then(_ => waitForWhitelistSync(ports, total, available), tryCount)

export async function createApiNodes(howMany, whitelist = true) {
  const ports = new Array(howMany).fill(0).map((_, i) => LEADER_PORT + i)
